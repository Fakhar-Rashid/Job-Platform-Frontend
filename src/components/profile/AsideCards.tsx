...
export function StatsCard({ profile }: { profile: Profile }) {
  return (
    <Card className="flex gap-9">
      <div>
        <b className="text-xl">${profile.stats.totalEarnings}</b>
        <div className="text-muted">Total earnings</div>
      </div>
      <div>
        <b className="text-xl">{profile.stats.totalJobs}</b>
...
export function PromoteCard({ profile, editable }: SectionProps) {
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3 border-t border-hair py-[9px]">
        <span>Availability badge</span>
        <span className="text-muted">{profile.availabilityBadge ? 'On' : 'Off'}</span>
      </div>
      <div className="flex items-center justify-between gap-3 border-t border-hair py-[9px]">
        <span>Boost your profile</span>
        <span className="text-muted">{profile.boostProfile ? 'On' : 'Off'}</span>
      </div>
    </CoreSection>
  );
}
...
export function VerificationsCard({ profile, editable }: SectionProps) {
  const rows: [string, boolean][] = [
    ...
    ['Military veteran', profile.militaryVeteran],
  ];
  return (
    <CoreSection
      title="Verifications"
      ...
      values={{
        ...
        militaryVeteran: profile.militaryVeteran,
      }}
      fields={[
    ...
    >
      {rows.map(([label, on]) => (
        <div className="flex items-center justify-between gap-3 border-t border-hair py-[9px]" key={label}>
          <span>{label}</span>
          {on ? (
            <span className="inline-flex items-center gap-1 text-[13px] text-verified">
  ...
  return (
    <CoreSection
      ...
      values={{
        ...
        openToContractToHire: profile.openToContractToHire,
      }}
      fields={[
  ...
  return (
    ...
    >
      {profile.videoIntroUrl ? (
        <a href={profile.videoIntroUrl} target="_blank" rel="noreferrer">
          Watch introduction
        </a>
      ) : (
  ...
  return (
    <Card>
      <div className="mb-3 flex items-start justify-between gap-3">
        <h3>Connects: {profile.connectBalance}</h3>
      </div>
